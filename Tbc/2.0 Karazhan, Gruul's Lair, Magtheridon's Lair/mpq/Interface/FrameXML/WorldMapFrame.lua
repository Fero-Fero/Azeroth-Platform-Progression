...
function WorldMapFrame_IsZoomOutBlocked()
	return GetCurrentMapContinent() == WORLDMAP_COSMIC_ID;
end
...
function WorldMapFrame_Update()
	...
	if ( not WorldMapFrame_IsZoomOutBlocked() ) then
...
function WorldMapZoomOutButton_OnClick()
	...
	elseif ( GetCurrentMapDungeonLevel() > 0 ) then
		ZoomOut();
	elseif ( continent == WORLDMAP_WORLD_ID ) then
		SetMapZoom(WORLDMAP_COSMIC_ID);
	elseif ( continent == WORLDMAP_COSMIC_ID ) then
		SetMapZoom(WORLDMAP_WORLD_ID);
	elseif ( continent == WORLDMAP_NORTHREND_ID ) then
		SetMapZoom(WORLDMAP_WORLD_ID);
	else
		SetMapZoom(WORLDMAP_WORLD_ID);
	end
