...
function AccountLogin_OnLoad(self)
	...
	AccountLoginVersion:SetFormattedText(VERSION_TEMPLATE, "Release", "2.4.3", "8606", "release", "Jul 15 2008");
...
function CinematicsFrame_OnLoad(self)
	-- GetClientExpansionLevel(): 0=Vanilla, 1=TBC, 2=WotLK, 3=Cata, ...
	-- Cap at 2 so only WoW + BC cinematics are offered on this TBC interface.
	local numMovies = math.min(GetClientExpansionLevel() or 0, 2);
	CinematicsFrame.numMovies = numMovies;
end

function CinematicsFrame_SetupButtons()
	if ( CinematicsFrame.numMovies < 2 ) then
		return;
	end
	...
	if ( numButtons > 0 and CinematicsBackground ) then
		CinematicsBackground:SetHeight(numButtons * 40 + 70);
	end
end
