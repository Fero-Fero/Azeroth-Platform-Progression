...
function CharacterCreate_OnLoad(self)
	...
	CharacterCreate_InstallCustomizationHooks();
...
function CharacterCreate_OnShow()
	...
	CharacterCreate_EnsureValidRace();
	CharacterCreate.selectedRace = GetSelectedRace();

	CharacterCreate_EnsureValidClass();
	CharacterCreate_RefreshClassSelection();
...
function CharacterCreate_IsHiddenRace(fileString)
	if ( not fileString ) then
		return false;
	end
	return HIDDEN_RACES[strupper(fileString)];
end

function CharacterCreate_IsHiddenRaceIndex(raceIndex)
	if ( not raceIndex ) then
		return false;
	end
	if ( HIDDEN_RACE_INDICES[raceIndex] ) then
		return true;
	end
	local races = { GetAvailableRaces() };
	for i=1, select("#", races), 3 do
		if ( (i - 1) / 3 + 1 == raceIndex ) then
			return CharacterCreate_IsHiddenRace(select(i+1, races));
		end
	end
	return false;
end

function CharacterCreate_IsHiddenRaceSelected()
	return CharacterCreate_IsHiddenRaceIndex(GetSelectedRace());
end

function CharacterCreate_SelectValidRace()
	local selectedRace = GetSelectedRace();
	if ( not CharacterCreate_IsHiddenRaceIndex(selectedRace) ) then
		CharacterCreate.selectedRace = selectedRace;
		return;
	end

	local races = { GetAvailableRaces() };
	for i=1, select("#", races), 3 do
		local raceIndex = (i - 1) / 3 + 1;
		if ( not CharacterCreate_IsHiddenRace(select(i+1, races)) ) then
			SetSelectedRace(raceIndex);
			CharacterCreate.selectedRace = raceIndex;
			return;
		end
	end
end

function CharacterCreate_EnsureValidRace()
	if ( CharacterCreate_IsHiddenRaceSelected() ) then
		CharacterCreate_SelectValidRace();
	end
end
...
function CharacterCreateEnumerateRaces(...)
	...
	for i=1, select("#", ...), 3 do
		...
		if ( not CharacterCreate_IsHiddenRace(fileString) ) then
			local raceIndex = (i - 1) / 3 + 1;
			coords = RACE_ICON_TCOORDS[strupper(fileString.."_"..gender)];
			button = _G["CharacterCreateRaceButton"..index];
			if ( not button or not coords ) then
				break;
			end
			...
			if ( select(i+2, ...) == 1 ) then
				...
				button.name = select(i, ...)
				button.tooltip = _G[strupper(fileString.."_".."DISABLED")];
			end
			index = index + 1;
		end
	end
	...
	for i=CharacterCreate.numRaces + 1, MAX_RACES, 1 do
		button = _G["CharacterCreateRaceButton"..i];
		if ( button ) then
			button:Hide();
		end
	end
...
function CharacterCreate_IsHiddenClass(fileString)
	if ( not fileString ) then
		return false;
	end
	local classFile = strupper(fileString);
	if ( HIDDEN_CLASSES[classFile] ) then
		return true;
	end
	local _, faction = GetFactionForRace(CharacterCreate.selectedRace);
	if ( faction == "Alliance" and classFile == "SHAMAN" ) then
		return true;
	end
	if ( faction == "Horde" and classFile == "PALADIN" ) then
		return true;
	end
	return false;
end

function CharacterCreate_ShouldShowClass(fileString, classIndex, available)
	if ( classIndex == DEATH_KNIGHT_CLASS_INDEX ) then
		return false;
	end
	if ( CharacterCreate_IsHiddenClass(fileString) ) then
		return false;
	end
	if ( available ~= 1 ) then
		return false;
	end
	local race = CharacterCreate.selectedRace;
	if ( race == 0 ) then
		race = GetSelectedRace();
	end
	return IsRaceClassValid(race, classIndex);
end

function CharacterCreate_IsDeathKnightSelected()
	local _, classFileName, classIndex = GetSelectedClass();
	local classFile = strupper(classFileName or "");
	classFile = gsub(classFile, "%s+", "");
	if ( classFile == "DEATHKNIGHT" or classIndex == DEATH_KNIGHT_CLASS_INDEX ) then
		return true;
	end
	local classes = { GetAvailableClasses() };
	for i=1, select("#", classes), 3 do
		if ( (i - 1) / 3 + 1 == classIndex and strupper(select(i+1, classes) or "") == "DEATHKNIGHT" ) then
			return true;
		end
	end
	return false;
...
function CharacterCreate_GetFirstAllowedClassIndex()
	if ( CharacterCreate.selectedRace == 0 ) then
		CharacterCreate.selectedRace = GetSelectedRace();
	end
	local classes = { GetAvailableClasses() };
	for j=1, select("#", classes), 3 do
		local classIndex = (j - 1) / 3 + 1;
		if ( CharacterCreate_ShouldShowClass(select(j+1, classes), classIndex, select(j+2, classes)) ) then
			return classIndex;
		end
	end
	for j=1, select("#", classes), 3 do
		local classIndex = (j - 1) / 3 + 1;
		if ( classIndex ~= DEATH_KNIGHT_CLASS_INDEX and not CharacterCreate_IsHiddenClass(select(j+1, classes)) and select(j+2, classes) == 1 ) then
			return classIndex;
		end
	end
	return 1;
...
function CharacterCreate_ForceOffDeathKnight()
	if ( not CharacterCreate_IsDeathKnightSelected() ) then
		return false;
	end
	SetSelectedClass(CharacterCreate_GetFirstAllowedClassIndex());
	return true;
end

function CharacterCreate_SelectValidClass()
	if ( CharacterCreate.selectedRace == 0 ) then
		CharacterCreate.selectedRace = GetSelectedRace();
	end

	if ( CharacterCreate_IsDeathKnightSelected() ) then
		CharacterCreate_ForceOffDeathKnight();
		return;
	end

	local _, classFileName, selectedClass = GetSelectedClass();
	local classes = { GetAvailableClasses() };
	local isValid = false;

	for i=1, select("#", classes), 3 do
		local classIndex = (i - 1) / 3 + 1;
		if ( classIndex == selectedClass ) then
			isValid = CharacterCreate_ShouldShowClass(select(i+1, classes), classIndex, select(i+2, classes));
			break;
		end
	end
...
function CharacterCreate_EnsureValidClass()
	CharacterCreate_ForceOffDeathKnight();
	CharacterCreate_SelectValidClass();
	CharacterCreate_ForceOffDeathKnight();
end

function CharacterCreate_InstallCustomizationHooks()
	if ( CharacterCreate_HooksInstalled ) then
		return;
	end
	CharacterCreate_HooksInstalled = true;

	if ( ResetCharCustomize ) then
		CharacterCreate_ResetCharCustomize = ResetCharCustomize;
		ResetCharCustomize = function()
			CharacterCreate_ResetCharCustomize();
			CharacterCreate_EnsureValidRace();
			CharacterCreate_EnsureValidClass();
		end
	end

	if ( RandomizeCharCustomization ) then
		CharacterCreate_RandomizeCharCustomization = RandomizeCharCustomization;
		RandomizeCharCustomization = function()
			CharacterCreate_RandomizeCharCustomization();
			CharacterCreate_EnsureValidRace();
			CharacterCreate_EnsureValidClass();
		end
	end

	if ( SetSelectedClass ) then
		CharacterCreate_SetSelectedClass = SetSelectedClass;
		SetSelectedClass = function(classIndex)
			if ( classIndex == DEATH_KNIGHT_CLASS_INDEX ) then
				classIndex = CharacterCreate_GetFirstAllowedClassIndex();
			end
			CharacterCreate_SetSelectedClass(classIndex);
		end
	end
end

function CharacterCreate_RefreshClassSelection()
	if ( CharacterCreate_IsHiddenRaceSelected() ) then
		CharacterCreate_EnsureValidRace();
	end
	CharacterCreate_EnsureValidClass();
...
function CharacterCreateEnumerateClasses(...)
	...
	for i=1, select("#", ...), 3 do
		local fileString = select(i+1, ...);
		local classIndex = (i - 1) / 3 + 1;
		if ( CharacterCreate_ShouldShowClass(fileString, classIndex, select(i+2, ...)) ) then
			if ( index > MAX_CLASSES_PER_RACE ) then
				break;
			end
			coords = CLASS_ICON_TCOORDS[strupper(fileString)];
	...
	CharacterCreate.numClasses = index - 1;
	for i=CharacterCreate.numClasses + 1, MAX_CLASSES_PER_RACE, 1 do
		button = _G["CharacterCreateClassButton"..i];
		if ( button ) then
			button:Hide();
		end
	end
...
function SetCharacterRace(id)
	...
	if ( CharacterCreate_IsHiddenRaceIndex(id) ) then
		CharacterCreate_SelectValidRace();
		id = GetSelectedRace();
		CharacterCreate.selectedRace = id;
	end
...
function CharacterClass_OnClick(id)
	...
	local classIndex = (button and button.classIndex) or id;
	PlaySound("gsCharacterCreationClass");
	if ( CharacterCreate_IsDeathKnightSelected() ) then
		return;
	end
	local _, classFileName, currClass = GetSelectedClass();
	if ( CharacterCreate_IsHiddenClass(classFileName) ) then
		return;
	end
	if ( currClass ~= classIndex and IsRaceClassValid(GetSelectedRace(), classIndex) ) then
...
function CharacterChangeFixup()
	if ( PAID_SERVICE_TYPE ) then
		for i=1, CharacterCreate.numClasses, 1 do
			local button = _G["CharacterCreateClassButton"..i];
			if ( CharacterCreate.selectedClass ~= button.classIndex ) then
				button:Disable();
		...
		for i=1, CharacterCreate.numRaces, 1 do
			local button = _G["CharacterCreateRaceButton"..i];
			local raceIndex = button.raceIndex;
			local allow = false;
			if ( PAID_SERVICE_TYPE == PAID_FACTION_CHANGE ) then
				local faction = GetFactionForRace(PaidChange_GetCurrentRaceIndex());
				if ( (raceIndex == PaidChange_GetCurrentRaceIndex()) or ((GetFactionForRace(raceIndex) ~= faction) and (IsRaceClassValid(raceIndex,CharacterCreate.selectedClass))) ) then
					allow = true;
				end
			...
			elseif ( PAID_SERVICE_TYPE == PAID_CHARACTER_CUSTOMIZATION ) then
				if ( raceIndex == CharacterCreate.selectedRace ) then
					allow = true
				end
			end
			if (not allow) then
