...
WORLDMAP_WORLD_ID = 0;
WORLDMAP_KALIMDOR_ID = 1;
WORLDMAP_EASTERN_KINGDOMS_ID = 2;
WORLDMAP_OUTLAND_ID = 3;
WORLDMAP_NORTHREND_ID = 4;
WORLDMAP_WINTERGRASP_ID = 502;

local WORLDMAP_DISABLED_CONTINENTS = {
	[WORLDMAP_OUTLAND_ID] = true,
	[WORLDMAP_NORTHREND_ID] = true,
};

function WorldMapFrame_IsContinentDisabled(continentID)
	return WORLDMAP_DISABLED_CONTINENTS[continentID];
end

function WorldMapFrame_IsZoomOutBlocked()
	return GetCurrentMapContinent() == WORLDMAP_WORLD_ID and GetCurrentMapZone() == WORLDMAP_WORLD_ID;
end

function WorldMapFrame_GetFallbackContinent()
	return WORLDMAP_WORLD_ID;
end

function WorldMapFrame_ValidateMap()
	local continentID = GetCurrentMapContinent();
	if ( continentID == WORLDMAP_COSMIC_ID ) then
		SetMapZoom(WORLDMAP_WORLD_ID);
		return true;
	end
	if ( WorldMapFrame_IsContinentDisabled(continentID) ) then
		SetMapZoom(WORLDMAP_WORLD_ID);
		return true;
	end
end
...
function WorldMapFrame_Update()
	...
	if ( IsZoomOutAvailable() and not WorldMapFrame_IsZoomOutBlocked() ) then
...
function WorldMapContinentsDropDown_Update()
	...
	if ( (GetCurrentMapContinent() == 0) or (GetCurrentMapContinent() == WORLDMAP_COSMIC_ID) or WorldMapFrame_IsContinentDisabled(GetCurrentMapContinent()) ) then
		UIDropDownMenu_ClearAll(WorldMapContinentDropDown);
	else
		local continentID = GetCurrentMapContinent();
		local dropdownIndex = 0;
		for i=1, select("#", GetMapContinents()), 1 do
			if ( not WorldMapFrame_IsContinentDisabled(i) ) then
				dropdownIndex = dropdownIndex + 1;
				if ( i == continentID ) then
					UIDropDownMenu_SetSelectedID(WorldMapContinentDropDown, dropdownIndex);
					break;
				end
			end
		end
...
function WorldMapZoomOutButton_OnClick()
	...
	if ( WorldMapFrame_IsZoomOutBlocked() ) then
		return;
	end
	if ( GetCurrentMapZone() ~= WORLDMAP_WORLD_ID ) then
	...
	elseif ( GetCurrentMapContinent() == WORLDMAP_COSMIC_ID ) then
		SetMapZoom(WORLDMAP_WORLD_ID);
	elseif ( GetCurrentMapContinent() == WORLDMAP_OUTLAND_ID or GetCurrentMapContinent() == WORLDMAP_NORTHREND_ID ) then
		SetMapZoom(WORLDMAP_WORLD_ID);
	else
		SetMapZoom(WORLDMAP_WORLD_ID);
	end
